...
public:
  HipoFileIterator(std::vector<std::string> &__filenames__,
                   std::vector<std::string> &__banknames__, int __batchsize__,
    ...
    separator = "_";
    item_indices =
        std::vector<int>(9); // NOTE: Only 6 types, but they only go up to 8 and
                             // it's easier to use the type int as the index.
    tags = __tags__;

    // Check length of filenames
    if (filenames.size() == 0)
      throw std::invalid_argument(
          "HipoFileIterator: length of filenames must be > 0");

    // Check batchsize
    if (batchsize <= 0)
      throw std::invalid_argument("HipoFileIterator: batch_size must be > 0");
    ...
    for (int i = 0; i < nbanks; i++) {
      ...
      std::vector<std::string> vec_entryname = std::vector<std::string>(
          0); // NOTE: IMPORTANT THAT THIS IS ZERO.  IF YOU SET TO NENTRIES YOU
              // MESS UP PUSHBACK AND GET A VECTOR TWICE AS LONG AS YOU WANT.
      for (int j = 0; j < nentries; j++) {
        ...
        item_index_map.insert(std::make_pair(bankname + separator + entryname,
                                             item_indices[entrytype]));
        item_type_map.insert(
            std::make_pair(bankname + separator + entryname, entrytype));
  ...
  } // HipoFileIterator() //NOTE: //TODO: Could also define a init method where
    // you specify bank entries and then parse in python... and could also
    // figure out how to include cuts....

  virtual ~HipoFileIterator() = default;

  /**
   * Open the next file in this.filenames if the end of this.filenames has not
   * yet been reached.
   * @return bool
   */
  bool open() {
  ...
  void reset() {
    try {
      vec_double =
          std::vector<std::vector<std::vector<double>>>(item_indices.at(5));
      vec_float =
          std::vector<std::vector<std::vector<float>>>(item_indices.at(4));
      vec_int = std::vector<std::vector<std::vector<int>>>(item_indices.at(3));
      vec_long =
          std::vector<std::vector<std::vector<long>>>(item_indices.at(8));
      vec_short =
          std::vector<std::vector<std::vector<int>>>(item_indices.at(2));
   ...
   * @param int tag
   */
  void setTags(int tag) { reader.setTags(tag); }
  ...
  bool next(int __counter__ = 0) {
    ...
    while (reader.next()) {
      ...
      for (int i = 0; i < nbanks; i++) {
        ...
        std::vector<std::string> entrynames =
            itemlist.at(i); // TODO: COULD ALSO COMPARE TIME WITH SCHEMALIST...
        ...
        for (int j = 0; j < nentries; j++) {
          ...
          switch (type) {
          case 1:
            vec_byte.at(entryindex).push_back(_getBytes(bank, name));
            break; // NOTE COULD INSERT INTO PREEXISTING VECTOR?  AND THEN
                   // INITIALIZE VECTOR WITH CORRECT DIMENSIONS ON FIRST TWO
                   // AXES...
          case 2:
            vec_short.at(entryindex).push_back(_getShorts(bank, name));
            break; // TODO: ALSO NEED TO DEAL WITH CASE WHERE SOME EVENTS DON'T
                   // HAVE ALL THE BANKS.
          case 3:
            vec_int.at(entryindex).push_back(_getInts(bank, name));
            break; // TODO: Make sure this will modify in place.
          case 4:
            vec_float.at(entryindex).push_back(_getFloats(bank, name));
            break; // TODO: Figure out how to add empty events for banks that
                   // don't occur in a given event... -> GetDoubles etc should
                   // return empty array if bank is empty in event and schema is
                   // still there so this should be fine, in principle.
          case 5:
            vec_double.at(entryindex).push_back(_getDoubles(bank, name));
            break;
          case 8:
            vec_long.at(entryindex).push_back(_getLongs(bank, name));
            break;
          default:
            throw pybind11::type_error(
                "HipoFileIterator: Invalid type " + std::to_string(type) +
                " for entry: " + entrynames.at(j) + "\n");
          }
        } // for (int j = 0; j<nentries; j++)
    ...
    if (open()) {
      return next(
          counter); // NOTE: Recursive function //NOTE: If you do not return
                    // here, the original function keeps going after the
                    // recursive call and returns false and so ends iteration.
    }
    // TODO: NOTE: THIS SHOULD NOT RAISE A STOP ITERATION BECAUSE YOU STILL NEED
    // TO GET THE BATCH DATA IN PYTHON
  ...
  int getType(std::string bankname, std::string item) {
    return item_type_map.at(
        bankname + separator +
        item); // NOTE: //TODO: COULD ADD pybind11::attribute_error here and try
               // block.
  }
   ...
   */
  std::vector<std::vector<double>> getDoubles(std::string bankname,
                                              std::string item) const noexcept {
   ...
   */
  std::vector<std::vector<float>> getFloats(std::string bankname,
                                            std::string item) const noexcept {
    int i = item_index_map.at(bankname + separator + item);
   ...
   */
  std::vector<std::vector<int>> getInts(std::string bankname,
                                        std::string item) const noexcept {
    int i = item_index_map.at(bankname + separator + item);
    return vec_int.at(i);
  }
   ...
   */
  std::vector<std::vector<long>> getLongs(std::string bankname,
                                          std::string item) const noexcept {
    int i = item_index_map.at(bankname + separator + item);
   ...
   * @return std::vector<std::vector<int>>
   */
  std::vector<std::vector<int>> getShorts(std::string bankname,
                                          std::string item) const noexcept {
    int i = item_index_map.at(bankname + separator + item);
   ...
   * @return std::vector<std::vector<int>>
   */
  std::vector<std::vector<int>> getBytes(std::string bankname,
                                         std::string item) const noexcept {
    int i = item_index_map.at(bankname + separator + item);
...
template <class HipoFileIteratorBase = HipoFileIterator>
class PyHipoFileIterator : public HipoFileIteratorBase {
public:
...
// NOTE: utils, benchmark classes throw compilation error if you try to use
// trampoline class.

template <class DatastreamBase = hipo::datastream>
class PyDatastream : public DatastreamBase {
public:
  ...
  long position() override {
    PYBIND11_OVERRIDE_PURE(long, DatastreamBase, position, );
  }
  long position(long pos) override {
    PYBIND11_OVERRIDE_PURE(long, DatastreamBase, position, pos);
  }
  void open(const char *filename) override {
    PYBIND11_OVERRIDE_PURE(void, DatastreamBase, open, filename);
  }
  int read(char *s, int size) override {
    PYBIND11_OVERRIDE_PURE(int, DatastreamBase, read, s, size);
  }
};

// NOTE: datastreamLocalFile, datastreamXrootd classes thorw compilation error
// if you try to use trampoline class.
...
template <class DictionaryBase = hipo::dictionary>
class PyDictionary : public DictionaryBase {
public:
  using DictionaryBase::DictionaryBase; // Inherit constructors
};

template <class StructureBase = hipo::structure>
class PyStructure : public StructureBase {
public:
  using StructureBase::StructureBase; // Inherit constructors
  void notify() override {
    PYBIND11_OVERRIDE_PURE(void, StructureBase, notify, );
  }
};

template <class BankBase = hipo::bank>
class PyBank : public PyStructure<BankBase> {
public:
...
// NOTE: readerIndex, reader, data, record classes throw compilation error if
// you try to use trampoline class.

template <class RecordBuilderBase = hipo::recordbuilder>
class PyRecordBuilder : public RecordBuilderBase {
public:
...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<HipoFileIterator, PyHipoFileIterator<>> hipofileiterator(
      m, "HipoFileIterator");
  hipofileiterator.def(py::init([](std::vector<std::string> &filenames,
                                   std::vector<std::string> &banknames,
                                   int batchsize, std::vector<int> &tags) {
    return new HipoFileIterator(filenames, banknames, batchsize, tags);
  }));
  ...
  hipofileiterator.def(
      "setTags",
      &HipoFileIterator::setTags); // TODO: ADD OPTION IN INIT TO SET TAGS...
  ...
  hipofileiterator.def_property_readonly("batchsize",
                                         &HipoFileIterator::getBatchsize);
  hipofileiterator.def_property_readonly("nbanks",
                                         &HipoFileIterator::getNBanks);
  hipofileiterator.def_property_readonly("index", &HipoFileIterator::getIndex);
  hipofileiterator.def_property_readonly("filenames",
                                         &HipoFileIterator::getFilenames);
  hipofileiterator.def_property_readonly("banknames",
                                         &HipoFileIterator::getBanknames);
  hipofileiterator.def_property_readonly("banks",
                                         &HipoFileIterator::getBanklist);
  hipofileiterator.def_property_readonly("types",
                                         &HipoFileIterator::getTypelist);
  hipofileiterator.def_property_readonly("items",
                                         &HipoFileIterator::getItemlist);
  hipofileiterator.def_property_readonly("reader",
                                         &HipoFileIterator::getReader);
  ...
  hipofileiterator.def("__repr__", // TODO: Test this function in python
                       [](HipoFileIterator &hfi) {
                         std::string r("HipoFileIterator");
                         return r;
                       });
  hipofileiterator.def("__eq__", // TODO: Test this function in python
                       [](HipoFileIterator *hfi1, HipoFileIterator *hfi2) {
                         return hfi1 == hfi2;
                       });
  ...
  benchmark.def(
      py::init([](const char *name) { return new hipo::benchmark(name); }));
  ...
  schema.def(py::init<>());
  schema.def(py::init([](const char *name, int __groupid, int __itemid) {
    return new hipo::schema(name, __groupid, __itemid);
  }));
  schema.def(
      py::init([](const hipo::schema &s) { return new hipo::schema(s); }));
  ...
  schema.def("getOffset",
             py::detail::overload_cast_impl<int, int, int>()(
                 &hipo::schema::getOffset, py::const_),
             "getOffset"); // NOTE:  Need this syntax for const functions!
  schema.def("getOffset",
             py::detail::overload_cast_impl<const char *, int, int>()(
                 &hipo::schema::getOffset, py::const_),
             "getOffset"); // NOTE:  Need this syntax for const functions!
  schema.def("getEntryType",
             py::detail::overload_cast_impl<int>()(&hipo::schema::getEntryType,
                                                   py::const_),
             "getEntryType"); // NOTE:  Need this syntax for const functions!
  schema.def("getEntryType",
             py::detail::overload_cast_impl<const char *>()(
                 &hipo::schema::getEntryType, py::const_),
             "getEntryType"); // NOTE:  Need this syntax for const functions!
  ...
  schema.def("__repr__", // TODO: Test this function in python
             [](hipo::schema &s) {
               std::string r("Schema : name = " + s.getName() +
                             " , schemaString = " + s.getSchemaString() + "\n");
               return r;
             });
  ...
  dictionary.def(
      "__eq__", // TODO: Test this function in python
  ...
  structure.def(py::init([](int __group, int __item, std::string &str) {
    return new hipo::structure(__group, __item, str);
  }));
  ...
  structure.def("__repr__", // TODO: Test this function in python
                [](hipo::structure &s) {
                  std::string r("Structure : group = ");
                  r += std::to_string(s.getGroup()) +
                       " , item = " + std::to_string(s.getItem()) +
                       " , type = " + std::to_string(s.getType()) +
                       " , length = " + std::to_string(s.getSize()) + "\n";
                  return r;
                });
  structure.def(
      "__eq__", // TODO: Test this function in python
  ...
  bank.def(py::init<>());
  bank.def(py::init(
      [](const hipo::schema &__schema) { return new hipo::bank(__schema); }));
  bank.def(py::init([](const hipo::schema &__schema, int __rows) {
    return new hipo::bank(__schema, __rows);
  }));
  ...
  bank.def("getInt",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getInt,
                                                      py::const_),
           "getInt");
  bank.def("getInt",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getInt, py::const_),
           "getInt");
  bank.def("getShort",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getShort,
                                                      py::const_),
           "getShort");
  bank.def("getShort",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getShort, py::const_),
           "getShort");
  bank.def("getByte",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getByte,
                                                      py::const_),
           "getByte");
  bank.def("getByte",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getByte, py::const_),
           "getByte");
  bank.def("getFloat",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getFloat,
                                                      py::const_),
           "getFloat");
  bank.def("getFloat",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getFloat, py::const_),
           "getFloat");
  bank.def("getDouble",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getDouble,
                                                      py::const_),
           "getDouble");
  bank.def("getDouble",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getDouble, py::const_),
           "getDouble");
  bank.def("getLong",
           py::detail::overload_cast_impl<int, int>()(&hipo::bank::getLong,
                                                      py::const_),
           "getLong");
  bank.def("getLong",
           py::detail::overload_cast_impl<const char *, int>()(
               &hipo::bank::getLong, py::const_),
           "getLong");

  bank.def(
      "putInt",
      py::detail::overload_cast_impl<int, int, int32_t>()(&hipo::bank::putInt),
      "putInt");
  bank.def("putInt",
           py::detail::overload_cast_impl<const char *, int, int32_t>()(
               &hipo::bank::putInt),
           "putInt");
  bank.def("putShort",
           py::detail::overload_cast_impl<int, int, int16_t>()(
               &hipo::bank::putShort),
           "putShort");
  bank.def("putShort",
           py::detail::overload_cast_impl<const char *, int, int16_t>()(
               &hipo::bank::putShort),
           "putShort");
  bank.def(
      "putByte",
      py::detail::overload_cast_impl<int, int, int8_t>()(&hipo::bank::putByte),
      "putByte");
  bank.def("putByte",
           py::detail::overload_cast_impl<const char *, int, int8_t>()(
               &hipo::bank::putByte),
           "putByte");
  bank.def(
      "putFloat",
      py::detail::overload_cast_impl<int, int, float>()(&hipo::bank::putFloat),
      "putFloat");
  bank.def("putFloat",
           py::detail::overload_cast_impl<const char *, int, float>()(
               &hipo::bank::putFloat),
           "putFloat");
  bank.def("putDouble",
           py::detail::overload_cast_impl<int, int, double>()(
               &hipo::bank::putDouble),
           "putDouble");
  bank.def("putDouble",
           py::detail::overload_cast_impl<const char *, int, double>()(
               &hipo::bank::putDouble),
           "putDouble");
  bank.def(
      "putLong",
      py::detail::overload_cast_impl<int, int, int64_t>()(&hipo::bank::putLong),
      "putLong");
  bank.def("putLong",
           py::detail::overload_cast_impl<const char *, int, int64_t>()(
               &hipo::bank::putLong),
           "putLong");
  ...
  bank.def("show",
           py::detail::overload_cast_impl<>()(&hipo::bank::show, py::const_),
           "show"); // NOTE:  Need this syntax for const functions!
  bank.def(
      "show",
      py::detail::overload_cast_impl<bool>()(&hipo::bank::show, py::const_),
      "show"); // NOTE:  Need this syntax for const functions!
  bank.def("printValue", &hipo::bank::printValue);
  bank.def("reset", &hipo::bank::reset);
  bank.def("notify", &hipo::bank::notify);

  bank.def_property_readonly(
      "bankSchema", &hipo::bank::getSchema); // NOTE: Not really necessary.
  bank.def_property_readonly("bankRows", &hipo::bank::getRows);

  bank.def(
      "__repr__", // TODO: Test this function in python
      [](hipo::bank &b) {
        std::string r("Bank : name = ");
        r += b.getSchema().getName() +
             " , rows = " + std::to_string(b.getRows()) + "\n";
        for (int i = 0; i < b.getSchema().getEntries(); i++) {
          std::string x("\t");
          x += b.getSchema().getEntryName(i) + " : ";
          r += x;
          for (int k = 0; k < b.getRows(); k++) {
            if (b.getSchema().getEntryType(i) < 4) {
              r += std::to_string(b.getInt(i, k)) +
                   " "; // std::string y("%8d ", b.getInt(i,k)); r += y; // %8d
            } else {
              if (b.getSchema().getEntryType(i) == 4) {
                r += std::to_string(b.getFloat(i, k)) +
                     " "; // std::string y("%8.5f ",b.getFloat(i,k)); r += y; //
                          // %8.5f
              }
              if (b.getSchema().getEntryType(i) == 5) {
                r += std::to_string(b.getDouble(i, k)) +
                     " "; // std::string y("%8.5f ",b.getDouble(i,k)); r += y;
                          // // %8.5f
              }
              if (b.getSchema().getEntryType(i) == 8) {
                r += std::to_string(b.getLong(i, k)) +
                     " "; // std::string y("%14ld ", b.getLong(i,k)); r += y; //
                          // %14ld
              }
            } // else
          }
  ...
  event.def("init",
            static_cast<void (hipo::event::*)(std::vector<char> &)>(
                &hipo::event::init),
            "init");
  event.def(
      "init",
      static_cast<void (hipo::event::*)(const char *, int)>(&hipo::event::init),
      "init");
  event.def("getStructure",
            static_cast<void (hipo::event::*)(hipo::structure &, int, int)>(
  ...
  event.def("getStructure",
            static_cast<void (hipo::event::*)(hipo::bank &)>(
                &hipo::event::getStructure),
            "getStructure");
  event.def(
      "read",
      static_cast<void (hipo::event::*)(hipo::bank &)>(&hipo::event::read),
      "read");
  event.def("addStructure", &hipo::event::addStructure);
  event.def("override", &hipo::event::override);
  event.def(
      "remove",
      static_cast<void (hipo::event::*)(hipo::bank &)>(&hipo::event::remove),
      "remove");
  event.def("remove",
            static_cast<void (hipo::event::*)(int, int)>(&hipo::event::remove),
            "remove");
  event.def("replace", &hipo::event::replace);

  event.def("add", &hipo::event::add);
  event.def("get",
            static_cast<void (hipo::event::*)(hipo::node &, int, int)>(
                &hipo::event::get),
            "get");

  event.def("getStructurePosition",
            static_cast<std::pair<int, int> (hipo::event::*)(int, int)>(
                &hipo::event::getStructurePosition),
            "getStructurePosition");
  event.def("getStructurePosition4",
            static_cast<std::pair<int, int> (hipo::event::*)(int, int)>(
                &hipo::event::getStructurePosition4),
  ...
  event.def_static("getStructurePosition_static",
                   static_cast<std::pair<int, int> (*)(const char *, int, int)>(
                       &hipo::event::getStructurePosition),
                   "getStructurePosition");
  event.def_static(
      "getStructure_static",
      static_cast<void (*)(const char *, hipo::structure &, int, int)>(
          &hipo::event::getStructure),
      "getStructure");
  event.def_static("get_static",
                   static_cast<void (*)(const char *, hipo::node &, int, int)>(
                       &hipo::event::get),
                   "get");
  event.def_static("getStructureNoCopy", &hipo::event::getStructureNoCopy);

  // event.def_property_readonly("databuffer", nullptr); //NOTE: Not really
  // necessary.

  event.def(
      "__repr__", // TODO: Test this function in python
      [](hipo::event &e) {
        std::string r("Event : size = ");
        r += std::to_string(e.getSize()) + "\n";
        int position = 16;
        int eventSize = *(reinterpret_cast<uint32_t *>(&e.getEventBuffer()[4]));
        while (position + 8 < eventSize) {
          uint16_t gid =
              *(reinterpret_cast<uint16_t *>(&e.getEventBuffer()[position]));
          uint8_t iid =
              *(reinterpret_cast<uint8_t *>(&e.getEventBuffer()[position + 2]));
          uint8_t type =
              *(reinterpret_cast<uint8_t *>(&e.getEventBuffer()[position + 3]));
          int length =
              *(reinterpret_cast<int *>(&e.getEventBuffer()[position + 4]));
          r += "\tgroup = " + std::to_string(gid) +
               " , item = " + std::to_string(iid) +
  ...
  py::class_<hipo::readerIndex> readerIndex(
      m, "ReaderIndex"); // NOTE: Can't use trampoline class because you get
                         // this error: "Cannot use an alias class with a
  ...
  readerIndex.def("getRecordEventNumber",
                  &hipo::readerIndex::getRecordEventNumber);
  ...
  // readerIndex.def_property_readonly("recordEvents", nullptr); //NOTE: Not
  // really necessary. readerIndex.def_property_readonly("recordPosition",
  // nullptr);
  readerIndex.def_property_readonly("currentRecord",
                                    &hipo::readerIndex::getRecordNumber);
  readerIndex.def_property_readonly("currentEvent",
                                    &hipo::readerIndex::getEventNumber);
  readerIndex.def_property_readonly("currentRecordEvent",
                                    &hipo::readerIndex::getRecordEventNumber);

  readerIndex.def("__repr__", [](hipo::readerIndex &r) {
    std::string x("Reader Index : ");
    x += "nrecords = " + std::to_string(r.getNRecords()) +
         " , record number = " + std::to_string(r.getRecordNumber()) +
         " , event number = " + std::to_string(r.getEventNumber()) + "\n";
    return x;
  });
  ...
  reader.def(
      py::init([](const hipo::reader &r) { return new hipo::reader(r); }));
  ...
  reader.def("setTags",
             static_cast<void (hipo::reader::*)(int)>(&hipo::reader::setTags),
             "setTags");
  reader.def("setTags",
             static_cast<void (hipo::reader::*)(std::vector<long>)>(
                 &hipo::reader::setTags),
             "setTags");
  reader.def("setVerbose", &hipo::reader::setVerbose);

  reader.def("hasNext", &hipo::reader::hasNext);
  reader.def("next", static_cast<bool (hipo::reader::*)()>(&hipo::reader::next),
             "next");
  reader.def("gotoEvent", &hipo::reader::gotoEvent);
  reader.def("gotoRecord", &hipo::reader::gotoRecord);
  reader.def(
      "next",
      static_cast<bool (hipo::reader::*)(hipo::event &)>(&hipo::reader::next),
      "next");

  reader.def("next",
             static_cast<bool (hipo::reader::*)(std::vector<hipo::bank> &)>(
                 &hipo::reader::next),
             "next");
  ...
  reader.def(
      "loadRecord",
      static_cast<bool (hipo::reader::*)(int)>(&hipo::reader::loadRecord),
      "loadRecord");
  reader.def("loadRecord",
             static_cast<bool (hipo::reader::*)(hipo::record &, int)>(
                 &hipo::reader::loadRecord),
             "loadRecord");
  reader.def("getEntries", &hipo::reader::getEntries);
  reader.def("getInt", &hipo::reader::getInt);
  reader.def("getFloat", &hipo::reader::getFloat);

  reader.def("__repr__", [](hipo::reader &r) {
    std::string x("Reader : ");
    x += "nrecords = " + std::to_string(r.getNRecords()) +
         " , entries = " + std::to_string(r.getEntries()) + "\n";
    return x;
  });
  ...
  record.def("readRecord",
             static_cast<void (hipo::record::*)(std::ifstream &, long, int)>(
                 &hipo::record::readRecord),
             "readRecord");
  record.def(
      "readRecord",
      static_cast<bool (hipo::record::*)(std::ifstream &, long, int, long)>(
          &hipo::record::readRecord),
      "readRecord");
  ...
  record.def_property_readonly("readBenchmark",
                               &hipo::record::getReadBenchmark);
  record.def_property_readonly("unzipBenchmark",
                               &hipo::record::getUnzipBenchmark);
  record.def_property_readonly("indexBenchmark",
                               &hipo::record::getIndexBenchmark);

  record.def("__repr__", [](hipo::record &r) {
    std::string x("Record : ");
  ...
  py::class_<hipo::recordbuilder, PyRecordBuilder<>> recordbuilder(
      m, "RecordBuilder");
  recordbuilder.def(py::init<>([]() { return new hipo::recordbuilder(); }));
  recordbuilder.def(py::init<>([](int maxEvents, int maxLength) {
    return new hipo::recordbuilder(maxEvents, maxLength);
  }));

  recordbuilder.def("addEvent",
                    static_cast<bool (hipo::recordbuilder::*)(hipo::event &)>(
                        &hipo::recordbuilder::addEvent),
                    "addEvent");
  recordbuilder.def(
      "addEvent",
      static_cast<bool (hipo::recordbuilder::*)(std::vector<char> &, int, int)>(
          &hipo::recordbuilder::addEvent),
      "addEvent");
  ...
  recordbuilder.def_property_readonly("bufferUserWordOne",
                                      &hipo::recordbuilder::getUserWordOne);
  recordbuilder.def_property_readonly("bufferUserWordTwo",
                                      &hipo::recordbuilder::getUserWordTwo);

  recordbuilder.def("__repr__", [](hipo::recordbuilder &r) {
    std::string x("Recordbuilder : ");
    x += "entries = " + std::to_string(r.getEntries()) +
         " , userWordOne = " + std::to_string(r.getUserWordOne()) +
         " , userWordTwo = " + std::to_string(r.getUserWordTwo()) + "\n";
    return x;
  });
  recordbuilder.def("__eq__", // TODO: Test this function in python
                    [](hipo::recordbuilder *r1, hipo::recordbuilder *r2) {
                      return r1 == r2;
                    });
  ...
  writer.def("addEvent",
             static_cast<void (hipo::writer::*)(hipo::event &)>(
                 &hipo::writer::addEvent),
             "addEvent");
  writer.def("addEvent",
             static_cast<void (hipo::writer::*)(std::vector<char> &, int)>(
                 &hipo::writer::addEvent),
             "addEvent");
  ...
  writer.def_property_readonly("writerDictionary",
                               &hipo::writer::getDictionary);

  writer.def("__repr__", [](hipo::writer &w) {
    std::string r("Writer : ");
    return r + "\n";
  });
